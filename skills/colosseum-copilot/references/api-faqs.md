...
`GET /faqs/:program/:id` returns `{ source, faq }` for a stable FAQ identity. Unknown identities return 404; invalid query/path values return 400. The endpoints share the search rate limit of 30 requests/minute per user.
