...
For "who has tried X", run two differently worded searches. Set `diversify: false` for exhaustive lists and paginate while `hasMore` is true. Repeat with `filters.acceleratorOnly: true`, without carrying over the winner filter, and name the returned accelerator cohorts. Merge resubmissions only after establishing project identity; preserve each hackathon and date.
