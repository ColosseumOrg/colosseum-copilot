...
| PAT issuance | 10 req/min | `/oauth/personal_access_token` (per IP) |
...
**Fail-closed:** If the concurrency limiter is temporarily unavailable, it returns `503 UNAVAILABLE` rather than allowing unlimited concurrency. This is transient — retry after a brief delay.
...
```bash
curl "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/filters" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT"
```
...
```bash
curl -X POST "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/search/projects" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT" \
...
**Facets** — aggregate tag distributions across the matched cohort:
...
```bash
curl -X POST "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/search/archives" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/archives/DOCUMENT_UUID?offset=0&maxChars=8000" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT"
```
...
```bash
curl "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/projects/by-slug/your-project-slug" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT"
```
...
```bash
curl -X POST "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/analyze" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/compare" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT" \
  -H "Content-Type: application/json" \
  -d '{
...
#### GET /colosseum_copilot/clusters/:clusterKey

Fetch cluster details by cluster key. Only use when a cluster key is present in results.

```bash
curl "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/clusters/v1-c12" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT"
```
...
**Archive search:**
- Keep queries to **3-6 focused keywords**. Too short (1-2 words) is vague; too long dilutes embedding similarity (e.g., use `"prediction markets governance"` not `"cypherpunks remailers anonymity mix networks privacy routing"`).
...
**Cohort analysis:**
- `clusters`, `problemTags`, `techStack`: these dimensions exist in the schema but may not be populated for all cohorts. If a dimension returns empty, try `tracks` or `problemTags` instead.
