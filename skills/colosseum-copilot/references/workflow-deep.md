...
or cohort comparisons. Answer those conversationally with targeted API calls.
...
```bash
curl -s -X POST "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/search/archives" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT" \
  -H "Content-Type: application/json" \
  -d '{
...
3. **Source filter:** Try restricting to a high-signal source (e.g., `"sources": ["solana_repo_issues"]` for protocol-level topics, `"sources": ["cypherpunks_mailing_list"]` for privacy/crypto primitives)
...
```bash
curl -s "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/projects/by-slug/<slug>" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT"
```

#### 2d. Cohort Analysis (Topic-Aware Routing)
...
```bash
curl -s -X POST "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/analyze" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT" \
  -H "Content-Type: application/json" \
  -d '{
...
Based on projects, archives, and cohort analysis, identify **2-3 distinct research angles** worth exploring further.
...
- Reference cohort data if available (e.g., "Only 4% of Breakout + Radar submissions addressed X")
...
> **Related Builder:** [Name] (`slug`, [Cohort/Batch]) is working on [overlap].
...
```bash
curl -s "$COLOSSEUM_COPILOT_API_BASE/colosseum_copilot/archives/<documentId>?offset=0&maxChars=8000" \
  -H "Authorization: Bearer $COLOSSEUM_COPILOT_PAT"
```
...
- **Underexplored areas**: Where prior art is thin
